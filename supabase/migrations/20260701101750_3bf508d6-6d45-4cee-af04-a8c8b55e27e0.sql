...
BEGIN
  IF NEW.truck_nominal_capacity < 0 OR NEW.tank_capacity_liters < 0
     ...
     OR NEW.tolerance_rate < 0 OR NEW.stock_before < 0 OR NEW.gauge_after < 0 THEN
  ...
  -- Recalcule les champs dépendants si non fournis ou pour garantir la cohérence
  NEW.stock_theoretical := COALESCE(NEW.stock_before, 0) + COALESCE(NEW.quantity_unloaded, 0);
  NEW.gauge_after := COALESCE(NEW.gauge_after, NEW.stock_theoretical);
  NEW.ecart := COALESCE(NEW.quantity_unloaded, 0) - COALESCE(NEW.quantity_to_unload, 0);
  NEW.depotage_ecart := COALESCE(NEW.gauge_after, 0) - NEW.stock_theoretical;
